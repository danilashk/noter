...
import { RealtimeCard } from '@/components/board/RealtimeCard'
import { Cursor } from '@/components/realtime/Cursor'
import { PresenceIndicator } from '@/components/realtime/PresenceIndicator'
import { ConnectionStatus } from '@/components/realtime/ConnectionStatus'

interface RealtimeBoardPageProps {
  sessionId: string
  userId: string
  userName: string
  userColor: string
...
export function RealtimeBoardPage({ sessionId, userId, userName, userColor }: RealtimeBoardPageProps) {
  ...
  const handleBoardClick = useCallback((e: React.MouseEvent) => {
    // Проверяем, что клик был именно по доске, а не по карточке
  ...
  const handleCreateCard = useCallback(async () => {
    ...
    try {
      await createCard(
        'Новая карточка',
        mousePosition.x - 100, // Центрируем карточку относительно клика
        mousePosition.y - 60
      )
      setShowAddButton(false)
    } catch (error) {
    ...
    <div className="h-screen bg-gradient-to-br from-blue-50 to-indigo-100 overflow-hidden relative">
        ...
        <AnimatePresence>
          {showAddButton && (
            <motion.button
              ...
              className="absolute z-20 bg-blue-500 hover:bg-blue-600 text-white rounded-full p-3 shadow-lg transition-colors"
              style={{
      ...
      <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 z-30">
        <div className="bg-white/90 backdrop-blur-sm rounded-lg px-4 py-2 text-sm text-gray-600 shadow-sm">
          Нажмите в любом месте доски, чтобы добавить карточку
        </div>
      </div>
    </div>
  )
